...
To install without the script: `git clone --branch v0.2.0 https://github.com/korengast/angelia && cd angelia && npm ci --ignore-scripts && npm run build && npm i -g --ignore-scripts "$(npm pack --ignore-scripts | tail -1)"`, then `angelia init`. Without the service, `angelia daemon` runs it in the terminal; it reads `~/.angelia/env` itself.
